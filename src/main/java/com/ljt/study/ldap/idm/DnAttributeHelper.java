 ...
 */
@Slf4j
public class DnAttributeHelper {
    ...
    public static <T> T mapFromAttributes(Attributes attrs, Class<T> clazz) {
        ...
        while (all.hasMoreElements()) {
            ...
            System.out.println(next.getID() + "-" + next.get());
        }
    ...
    private static Map<String, Field> getFieldMap(Class<?> clazz) {
        ...
        ReflectionUtils.doWithFields(clazz, field -> {
            String attr = null;
            DnAttribute annotation = field.getAnnotation(DnAttribute.class);
            if (Objects.isNull(annotation) || !StringUtils.hasLength(annotation.value())) {
                attr = field.getName();
            } else {
